...
module Material
  ...
  class Distributor
    ...
    def material_for(pipeline)
      ...
      material = []
      material.push(GitMaterial.new(name: 'material1', url: "#{@setup.git_repository_host}/git-repo-#{pipeline}", destination: 'repo')) if suffix.between?(@git.begin, @git.end)
    ...
    def material_for_config_repo(pipeline)
      suffix = pipeline.gsub(/[^0-9]/, '').to_i
      material = []
      material.push(ConfigRepoGitMaterial.new(name: 'material1', url: "#{@setup.git_repository_host}/git-repo-#{pipeline}", destination: 'repo')) if suffix.between?(@git.begin, @git.end)
